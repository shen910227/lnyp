	...
	if(confirmOrder.proList[0].isO2O) {
		...
		ai.post({
			...
			load: function(json) {
				...
				confirmOrder.changeAddr = json.Data[0];
			}
		})
	...
	ai.post({
		...
		load: function(json) {
			...
			if(json.Data && json.Data.length > 0) {
				...
				coupon__main.parentElement.setAttribute('num', 0);
			}
		}
	})
}
		...
		ai.post({
			...
			load: function(json) {
				if(json.ResultCode == 1000) {
					...
					confirmOrder.eleCouponsFor(startTime, OrderNo, Mobile, eleCoupons);
				} else {
					confirmOrder.confirmBtnEvt(OrderNo, eleCoupons, 'fail');
				}
			}
		})
	})
...
//循环检查电子券支付结果
confirmOrder.eleCouponsFor = function(obj, OrderNo, Mobile, Amount) {
	var now = new Date();
	if((now - obj) / 1000 < 120) {
		ai.post({
			...
			load: function(json) {
				if(json.ResultCode == 1000) {
					...
					setTimeout(function() {
						confirmOrder.eleCouponsFor(obj, OrderNo, OrderNo, Amount);
					}, 5000);
				}
			}
		})
...
confirmOrder.confirmBtnEvt = function(OrderNo, Amount, TorF) {
	var eleCouponsBtn = document.getElementsByClassName('eleCouponsPay__btn')[0],
		...
		eleCouponsPay__fail = document.getElementsByClassName('eleCouponsPay__fail')[0],
		eleCouponsPay__loading = document.getElementsByClassName('eleCouponsPay__loading')[0];
	if(TorF == 'fail') {
		...
		eleCouponsPay__loading.style.display = 'none';
...
confirmOrder.addEvt = function() {
	...
	couponClick.addEventListener('click', function() {
		if(couponClick.getAttribute('num') != 0) {
